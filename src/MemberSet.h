...
#ifndef COMMON_SRC_INCLUDE_EBBRT_MEMBERSET_H_
#define COMMON_SRC_INCLUDE_EBBRT_MEMBERSET_H_
...
/* MemberSet<T> template defines a type, T, which uses the ZooKeeper class
 * to manage global member set. Action hooks are provided to
 * respond to set joins/quits.
 */
...
#endif // COMMON_SRC_INCLUDE_EBBRT_MEMBERSET_H_

// void GlobalCounterRoot::update_member_list_(){
//  auto members = ebbrt::zkglobal_id_map->List(id_).Block().Get();
//  {
//    std::lock_guard<std::mutex> guard(m_);
//    char mynodeid[20];
//    sprintf(mynodeid, "%d", ebbrt::runtime::AllocationId());
//    member_list_.clear();
//    for (auto &nodeid : members) {
//      if (nodeid != std::string(mynodeid)){
//        auto fn = ebbrt::zkglobal_id_map->Get(id_, nodeid);
//        auto n = fn.Block().Get();
//        member_list_.emplace(std::make_pair(nodeid,
//        ebbrt::Messenger::NetworkId::FromBytes(
//            (const unsigned char *)n.c_str(), 4)));
//
//      }
//    }
//  }
//  ebbrt::kprintf("Member list length = %d\n", member_list_.size());
//}

// void repeat_watcher_(EbbId id, std::string path, int type,
//                  std::function<void()> func){
//  auto f = [this,id, path, type, func]() {
//    auto f = func;
//    func();
//    this->repeat_watcher_(id, path, type, std::move(f));
//  };
//  ebbrt::zkglobal_id_map->SetWatcher(
//      id, new ebbrt::ZKGlobalIdMap::WatchEvent(type, std::move(f)), path);
//};
